  ...
  return {
    ...
    ctaButton: function (content, classes, type = 'button', href = '') {
      ...
      return `${href ? '<a href=' + href + '>' : ''
        }<button type="${type}" class="cursor-pointer text-2xl md:text-3xl px-3 py-2 text-balance font-medium rounded-md ${this.ctx.colors.buttonDefault.text || this.ctx.colors.buttonCustom.text
        } ${this.ctx.colors.buttonDefault.textHover || this.ctx.colors.buttonCustom.textHover} ${this.ctx.colors.buttonDefault.bg || this.ctx.colors.buttonCustom.bg
        } ${this.ctx.colors.buttonDefault.bgHover || this.ctx.colors.buttonCustom.bgHover} ${this.ctx.colors.buttonDefault.border || this.ctx.colors.buttonCustom.border
    ...
    heading: function (content, subheading, classes = '', anchorLink = true) {
      const h3 = subheading
        ? `<h3 class="${this.ctx.colors.headingsCustom || this.ctx.colors.headingsDefault
    ...
    youtube: function (vid, vtitle, vclasses, vratio, vstart) {
      ...
      return `<div class="${classes}" >
        <div id="${slug}" style="position:relative; width:100%; padding-bottom:${padding}%"><iframe style="position:absolute; top:0; right:0; bottom:0; left:0; width:100%; height:100%" width="100%" height="100%" title="${title}" src="https://www.youtube.com/embed/${slug}${start ? `?start=${start}` : ''
